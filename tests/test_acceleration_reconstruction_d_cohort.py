...
import scripts.run_experiment_d as runner
...
def _config(*, split: UserSplitConfig | None = None):
    return replace(experiment_d_config(test_source="raw"), split=split or UserSplitConfig())


def test_modern_reference_inherits_exclusion_and_saved_assignment_policy() -> None:
...
def test_local_d_cohort_selection_conflicts_with_reference(
    ...
    with pytest.raises(ValueError, match="Experiment D local"):
...
def test_saved_a_assignment_policy_allows_eligible_but_unassigned_users() -> None:
    ...
    assert contract.require_all_users_assigned is False
    assert contract.eligible_users == tuple(f"user_{index}" for index in range(5))
    assert set(result.sample_manifest["user"]) == {
        "user_0",
        "user_1",
        "user_2",
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: NormalizationStats(
            ...
            std=np.ones(3),
            valid_time_points=1,
            fitted_on="mixed:train",
        ),
    )

    def build_loaders(*args, **kwargs):
        loader_calls.append(
            (kwargs["train_source"], kwargs["val_source"], kwargs["test_source"])
        )
        return {
            "train": object(),
            "train_eval": object(),
    ...
    monkeypatch.setattr(runner, "build_split_loaders", build_loaders)
...
def test_modern_run_persists_cohort_identity_and_keeps_mixed_domains(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    loader_calls = _patch_d_execution(monkeypatch, data=data)
    monkeypatch.setattr(runner, "load_checkpoint", lambda *args, **kwargs: checkpoint)

    result = runner.run_experiment_d(
    ...
    assert loader_calls == [("mixed", "mixed", "raw"), ("mixed", "mixed", "reconstruction")]
    ...
    assert provenance["normalization"]["fitted_on"] == "mixed:train"
...
def test_allow_new_split_records_standalone_local_cohort_without_reference(
    ...
    _patch_d_execution(monkeypatch, data=data)
    ...
    saved = read_checkpoint(result.checkpoint_path)
    assert saved["cohort_source"] == "standalone_no_reference"
    assert saved["excluded_users"] == ["user_3"]
    assert saved["eligible_users"] == ["user_0", "user_1", "user_2"]
    assert saved["reference_identity"] is None
