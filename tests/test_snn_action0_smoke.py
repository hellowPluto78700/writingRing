...
def _write_package(
    ...
    *,
    ...
    values = np.zeros((2, padded_length, channel_count), dtype=np.float32)
    values[0, :, :event_channel_count] = 1.0
    values[1, :, :event_channel_count] = 0.5
...
def test_dry_run_accepts_all_thirty_polarity_split_event_channels(tmp_path: Path) -> None:
    for user in ("user_0", "user_1", "user_2"):
        _write_package(tmp_path, user, channel_count=36)
    _write_metadata(tmp_path, channel_count=36)
