...
def test_phase_and_capacity_control_have_equal_parameter_count() -> None:
    ...
    phase = exp803.Exp803Net(
        "l1_fixed250_l2_whole", n_classes=12, fs=64.0, n_steps=256, bin_steps=16
    )
    torch.manual_seed(1)
...
def test_phase_head_changes_weight_with_absolute_bin() -> None:
    model = exp803.Exp803Net(
        "l1_fixed250_l2_whole", n_classes=2, fs=64.0, n_steps=32, bin_steps=16
    )
...
def test_plan_and_notebook_contract() -> None:
    ...
    assert "l1_fixed250_l2_whole" in plan
    assert "l1_capacity_no_phase_l2_whole" in plan
