...
# Rockpool is only needed by the legacy SynNetRP path.  Do not import it while
# importing the local snnTorch SynNet baseline: Rockpool deployment extras may
# be absent even when local training dependencies are installed.
def _load_rockpool_synnet():
    try:
        from .rockpool_nn_networks_synnet import SynNet as rockpool_synnet
        ...
        try:
            ...
            raise ImportError(
                "SynNetRP requires the optional Rockpool deployment dependencies; "
                "local SynNet does not"
            ) from error
    return rockpool_synnet, bitshift_to_tau


def is_rockpool_synnet(model):
    """Return whether a model was constructed through the optional SynNetRP path."""

    return bool(getattr(model, "_writingring_rockpool_synnet", False))
...
def createModel(
    ...
    **kwargs
    ...
    if network_type == 'SynNetRP':
        model = SynNetRockPool(
            inputSize=inputSize,
        ...
        try: model_class = {'LSTMNet':LSTMNet, 'LSTMNetANN':LSTMNetANN, 'RSynNet':RSynNet,
                            'SynNet':SynNet, 'ANNMLP':ANNMLP, 'SNNMLP': SNNMLP,
...
class SynNet(nn.Module):
    def __init__(self, inputSize, outputSize=1, hiddenSizes=[24,24,24], sampleFreq=64,
        ...
        # Total spike record
        self.spkTotal = None
        self.spike_neuron_count = sum(hiddenSizes) + outputSize
    ...
    def forward(self, x, valid_mask=None):
        ...
        spk4_rec = torch.stack(spk4_rec, dim=1)
        ...
        if valid_mask is None:
            ...
            if valid_mask.shape != x.shape[:2]:
                raise ValueError(
                    "valid_mask must have shape (batch_size, time_steps); "
                    f"expected {tuple(x.shape[:2])}, got {tuple(valid_mask.shape)}"
                )
            mask = valid_mask.unsqueeze(-1).to(
                device=spk1_rec.device,
                dtype=spk1_rec.dtype,
            )
            self.spkTotal = (
                (spk1_rec * mask).sum()
                + (spk2_rec * mask).sum()
...
def SynNetRockPool(inputSize, outputSize=1, hiddenSizes=[24,24,24], sampleFreq=64,
                   shiftSyn=1, shiftMem=1, tausFactor=1, timeResolution=1/64, **kwargs):
    """Construct the optional Rockpool SynNet only when that architecture is selected."""

    rockpool_synnet, bitshift_to_tau = _load_rockpool_synnet()

    class _SynNetRockPool(rockpool_synnet):
        def __init__(self):
            super().__init__(
    ...
    model = _SynNetRockPool()
    model._writingring_rockpool_synnet = True
    return model
