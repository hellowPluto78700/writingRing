...
Board-assisted policy in this version intentionally differs from label-only
segmentation in three ways:

* long label intervals remain eligible; the *final Board-assisted window* is
  instead required to be at most ``maximum_segment_duration_us`` (5 s by
  default);
* a valid press/lift pair may be carried into the following label when the
  press is at most ``carry_in_press_lookback_us`` before that label and the
  pair is the immediate press-before/lift-after event pair around the label;
* for the first exportable segment of a recording whose ``*_board_0.gz`` is a
  serialized empty list, a strong alignment-grade Ring transient between the
  label and the first real aligned Board frame can recover the start boundary.
"""
...
_CARRY_IN_RESOLUTION: Final[str] = "carried_into_next_label"
_FIRST_SEGMENT_PEAK_SKIP_REASON: Final[str] = (
    "first_segment_preboard_strong_peak_not_found"
)
_MAXIMUM_SEGMENT_SKIP_REASON: Final[str] = "final_segment_duration_gt_5s"
...
class BoardEventSegmentationConfig:
    ...
    ``maximum_segment_duration_us`` applies to the final resolved
    Board-assisted window, not to the distance between adjacent labels.
    """
...
class _CandidateWindow:
    ...
    carry_in_start: bool = False
    transient_start_recovered: bool = False
...
def segment_recording_by_aligned_board_events(
    *,
    ...
    transient_score: np.ndarray | None = None,
    leading_board_chunk_empty: bool = False,
    first_aligned_board_frame_us: float | None = None,
    ...
    Board-assisted label filtering deliberately ignores the label-only 5 s
    maximum interval rule.  The 5 s default is enforced against every final
    resolved Board-assisted segment instead.
    """
    ...
    events, touch_pairs = _classify_crossing_touches(
        ...
        touch_pairs,
        markers,
        policy=config.crossing_touch_policy,
        carry_in_press_lookback_us=config.carry_in_press_lookback_us,
    )

    # Board-assisted mode keeps the label-only "wrong" and too-close rules,
    # but intentionally does not reject a label merely because the following
    # label is more than 5 s away.  The actual final Board-derived segment is
    # checked against maximum_segment_duration_us below.
    label_reasons = _board_label_start_skip_reasons(
        markers,
    ...
    for index, label in enumerate(markers):
        ...
        start = max(
            float(boundary_timestamps[0]),
            first_press - config.pre_press_context_us,
        )
        end = min(
            float(boundary_timestamps[-1]),
            last_lift + config.post_lift_context_us,
        )
        candidates[index] = _CandidateWindow(
            ...
            provisional_start_us=start,
            provisional_end_us=end,
            ...
            final_end_us=end,
            carry_in_start=(
                str(eligible.iloc[0]["crossing_resolution"])
                == _CARRY_IN_RESOLUTION
            ),
        )

    # Special first-segment recovery for a serialized-empty board_0 chunk.
    if candidates and leading_board_chunk_empty and first_aligned_board_frame_us is not None:
        first_index = min(candidates)
        first_candidate = candidates[first_index]
        if first_candidate.label.timestamp_us < first_aligned_board_frame_us:
            recovered = _recover_first_segment_start_from_transient_peak(
                first_candidate,
                transient_score=transient_score,
                boundary_timestamps_us=boundary_timestamps,
                first_aligned_board_frame_us=first_aligned_board_frame_us,
                pre_context_us=config.pre_press_context_us,
            )
            if not recovered:
                skipped[first_index] = _skipped(
                    first_candidate.label,
                    first_index,
                    first_candidate.next_label,
                    _FIRST_SEGMENT_PEAK_SKIP_REASON,
                    analysis=analyses[first_index],
                )
                del candidates[first_index]

    # Resolve neighboring contexts.  A carry-in segment uses an asymmetric
    # start rule: keep the previous segment unchanged and, only when the
    # nominal press-0.2 s start intrudes into it, place the new start at the
    # midpoint between previous.final_end and the carry-in press.
    while True:
        carry_in_failures = _resolve_window_collisions(candidates)
        if not carry_in_failures:
            break
        for index, reason in carry_in_failures.items():
    ...
    for index in sorted(candidates):
        ...
        # Unified Board-assisted quality gate: every final segment, regardless
        # of label spacing or boundary-recovery path, must be <= 5 s by default.
        final_duration_us = candidate.final_end_us - candidate.final_start_us
        if final_duration_us > config.maximum_segment_duration_us:
            skipped[index] = _skipped(
                candidate.label,
                index,
                candidate.next_label,
                _MAXIMUM_SEGMENT_SKIP_REASON,
                analysis=analyses[index],
            )
            continue
...
def _prepare_success_recording_segmentation(
    *,
    ...
    leading_board_chunk_empty: bool,
    ...
    try:
        ...
        first_aligned_board_frame_us = float(
            prepared.frames.iloc[0]["frame_timestamp_raw"]
        ) + float(offset.boundary_offset_us)
    except BoardEventSegmentationError as error:
        raise _RecordingLocalSegmentationFailure(
    ...
    try:
        recording_result = segment_recording_by_aligned_board_events(
            ...
            boundary_timestamps_us=boundary_timestamps_us,
            transient_score=transient_score,
            leading_board_chunk_empty=leading_board_chunk_empty,
            first_aligned_board_frame_us=first_aligned_board_frame_us,
        )
...
def _build_aligned_recording_artifacts(
    ...
    *,
    ...
    for recording in ordered_recordings:
        ...
        leading_board_chunk_empty = (
            0 in tuple(board.validation.empty_leading_chunk_indices)
        )
...
def _aggregate_and_write_aligned_outputs(
    ...
    *,
    ...
    for artifact in artifacts:
        sample_by_label = {
            sample.source_label_index: sample for sample in artifact.result.samples
        }
        skipped_by_label = {
            sample.source_label_index: sample for sample in artifact.result.skipped
        }
        ...
        event_rows.extend(
            _board_event_rows(
                artifact,
                global_segment_start=segment_index - len(artifact.result.samples),
            )
        )
...
def _combined_label_skip_reasons(
    ...
    if len(reasons) != len(labels):
        raise BoardEventSegmentationError(
            "recording result has incompatible label skip reasons"
        )
    ...
    *,
    ...
    base: dict[str, object] = {
        ...
        "skip_reason": (
            None
            if sample is not None
            else (skipped.skip_reason if skipped else "not_processed")
        ),
        ...
        "next_label_timestamp_us": (
            None if next_label is None else next_label.timestamp_us
        ),
        ...
        "canonical_offset_projection_success": (
            artifact.offset.canonical_offset_projection_success
        ),
        ...
        "acceleration_semantics": (
            _acceleration_semantics(gravity_method)
            if is_raw and gravity_method
            else None
        ),
        ...
        base.update(
            {
                ...
                "final_segment_duration_us": None,
                ...
                "accepted_crossing_touch_pair_count": (
                    0
                    if skipped is None
                    else skipped.accepted_crossing_touch_pair_count
                ),
                "rejected_crossing_touch_pair_count": (
                    0
                    if skipped is None
                    else skipped.rejected_crossing_touch_pair_count
                ),
    ...
    base.update(
        {
            ...
            "accepted_crossing_touch_pair_count": (
                sample.accepted_crossing_touch_pair_count
            ),
            "rejected_crossing_touch_pair_count": (
                sample.rejected_crossing_touch_pair_count
            ),
...
def _board_event_rows(
    ...
    for event in artifact.result.aligned_board_events.itertuples(index=False):
        ...
        if channel is not None:
            for local_segment_index, sample in enumerate(samples):
                if (
                    _optional_int(event.assigned_label_index)
                    != sample.source_label_index
                ):
                    continue
                boundary_start_us = (
                ...
                local_index = int(
                    np.searchsorted(segment_times, timestamp, side="left")
                )
                if (
                    local_index < sample.sample_count
                    and sample.board_event_targets[local_index, channel]
                ):
        ...
        rows.append(
            {
                ...
                "crosses_next_label_timestamp": bool(
                    event.crosses_next_label_timestamp
                ),
...
def _event_target_channel(event: object) -> int | None:
    if (
        bool(event.incomplete_touch)
        or _optional_int(event.assigned_label_index) is None
    ):
        return None
    ...
    *,
    ...
    for artifact in artifacts:
        ...
        for sample in artifact.result.samples:
            boundary_counts[sample.boundary_source] = (
                boundary_counts.get(sample.boundary_source, 0) + 1
            )
    ...
    return {
        ...
        "alignment_time_axis": {
            ...
            "sampling_rate_hz": (
                artifacts[0].offset.alignment_time_axis_sampling_rate_hz
            ),
        ...
        "minimum_label_interval_us": config.minimum_label_interval_us,
        "maximum_final_segment_duration_us": config.maximum_segment_duration_us,
        "carry_in_press_lookback_us": config.carry_in_press_lookback_us,
        "long_label_intervals_allowed": True,
        "first_segment_empty_board_transient_recovery": True,
        "source_label_count": int(
            sum(len(artifact.labels) for artifact in artifacts)
        ),
        "exported_segment_count": len(samples),
        "skipped_segment_count": int(
            sum(len(artifact.result.skipped) for artifact in artifacts)
        ),
        "skipped_segment_counts_by_reason": dict(sorted(skip_counts.items())),
        "valid_touch_pair_count": (
            int(pair_table["valid_touch"].astype(bool).sum())
            if not pair_table.empty
            else 0
        ),
        "transient_touch_pair_count": (
            int(pair_table["transient"].astype(bool).sum())
            if not pair_table.empty
            else 0
        ),
        "incomplete_touch_pair_count": (
            int(pair_table["incomplete_touch"].astype(bool).sum())
            if not pair_table.empty
            else 0
        ),
        "crossing_touch_pair_count": (
            int(pair_table["crossing_touch"].astype(bool).sum())
            if not pair_table.empty
            else 0
        ),
        "accepted_crossing_touch_pair_count": (
            int(
                (
                    pair_table["crossing_resolution"]
                    == "accepted_before_next_press"
                ).sum()
            )
            if not pair_table.empty
            else 0
        ),
        "carry_in_touch_pair_count": (
            int((pair_table["crossing_resolution"] == _CARRY_IN_RESOLUTION).sum())
            if not pair_table.empty
            else 0
        ),
        "rejected_crossing_touch_pair_count": (
            int(
                ...
                .isin(
                    (
                        "rejected_overlaps_next_press",
                        "rejected_next_press_missing",
                    )
                )
                .sum()
            )
        ...
        "recordings_without_verification": [
            ...
            for artifact in artifacts
            if not artifact.verification_path.is_file()
            or artifact.verification_path.stat().st_size == 0
        ],
        ...
        "alignment_input_hash_match_verified": (
            ...
            and all(
                artifact.offset.feature_values_sha256
                == artifact.feature_input.values_sha256
                and artifact.offset.feature_metadata_sha256
                == artifact.feature_input.metadata_sha256
                and artifact.offset.timestamp_sha256
                == artifact.feature_input.timestamps_sha256
                for artifact in artifacts
            )
        ),
        ...
        "gravity_removal": {
            ...
            "sampling_rate_hz": (
                None
                if not raw_gravity
                or gravity_config.gravity_removal_method == "raw"
...
def _validate_aggregate_arrays(
    ...
    *,
    ...
    if (
        ...
        raise BoardEventSegmentationError(
            "aligned aggregate arrays have invalid channel shapes"
        )
    if len(labels) != len(lengths) or len(offsets) != len(labels) + 1:
        raise BoardEventSegmentationError(
            "aligned aggregate arrays have inconsistent segment counts"
        )
    if (
        not np.array_equal(np.diff(offsets), lengths)
        or int(offsets[-1]) != len(raw_imu)
    ):
        raise BoardEventSegmentationError(
            "aligned aggregate offsets do not delimit raw IMU"
        )
...
def _board_label_start_skip_reasons(
    labels: Sequence[SegmentLabel],
    *,
    minimum_label_interval_us: float,
) -> tuple[str | None, ...]:
    """Apply Board-mode label validity without a maximum label-gap rule.

    ``wrong`` remains invalid.  An adjacent interval shorter than the minimum
    invalidates both neighboring starts, matching label-only semantics.  Long
    label intervals remain eligible because Board events determine the actual
    segment boundaries; the final segment duration is checked separately.
    """

    reasons: list[str | None] = [None] * len(labels)
    for index, marker in enumerate(labels):
        if marker.label.strip().casefold() == "wrong":
            reasons[index] = "label_is_wrong"
    for index in range(len(labels) - 1):
        interval = labels[index + 1].timestamp_us - labels[index].timestamp_us
        if interval < minimum_label_interval_us:
            _set_board_skip_reason(
                reasons, index, "adjacent_interval_lt_0.1s"
            )
            _set_board_skip_reason(
                reasons, index + 1, "adjacent_interval_lt_0.1s"
            )
    return tuple(reasons)


def _set_board_skip_reason(
    reasons: list[str | None], index: int, reason: str
) -> None:
    """Keep an explicit ``wrong`` label reason when several rules apply."""
...
def _analyze_label_interval(
    *,
    ...
    ordered = lift > press
    owned = (
        touch_pairs["assigned_label_index"].astype("Int64")
        == source_label_index
    ).fillna(False)
    eligible_mask = (
        owned
    ...
    if next_label is not None:
        event_interval &= (
            events["aligned_event_timestamp_us"] < next_label.timestamp_us
        )
    else:
    ...
    return LabelIntervalAnalysis(
        ...
        crossing_touch_pair_count=int(
            np.count_nonzero(
                in_interval
                & touch_pairs["crosses_next_label_timestamp"].astype(bool)
            )
        ),
        accepted_crossing_touch_pair_count=int(
            np.count_nonzero(
                in_interval
                & (
                    touch_pairs["crossing_resolution"]
                    == "accepted_before_next_press"
                )
            )
        ),
...
def _recover_first_segment_start_from_transient_peak(
    candidate: _CandidateWindow,
    *,
    transient_score: np.ndarray | None,
    boundary_timestamps_us: np.ndarray,
    first_aligned_board_frame_us: float,
    pre_context_us: float,
) -> bool:
    """Recover an empty-leading-Board first-segment start from a strong peak.

    Only alignment-grade peak regions strictly inside
    ``(label_timestamp, first_aligned_board_frame)`` are eligible.  The most
    prominent region wins, then the larger peak height, then the earlier peak
    timestamp.  No raw-score/ordinary-local-maximum fallback is used.
    """

    if transient_score is None:
        return False
    try:
        score = np.asarray(transient_score, dtype=np.float64)
    except (TypeError, ValueError):
        return False
    if (
        score.ndim != 1
        or len(score) != len(boundary_timestamps_us)
        or not np.isfinite(score).all()
    ):
        return False
    label_us = float(candidate.label.timestamp_us)
    board_start_us = float(first_aligned_board_frame_us)
    if not label_us < board_start_us:
        return False
    try:
        peaks = detect_transient_peak_regions(score, boundary_timestamps_us)
    except EventAlignmentError:
        return False
    eligible = peaks.loc[
        (peaks["peak_timestamp"] > label_us)
        & (peaks["peak_timestamp"] < board_start_us)
    ].copy()
    if eligible.empty:
        return False
    selected = eligible.sort_values(
        ["prominence", "height", "peak_timestamp"],
        ascending=[False, False, True],
        kind="mergesort",
    ).iloc[0]
    peak_us = float(selected["peak_timestamp"])
    recovered_start = max(
        float(boundary_timestamps_us[0]), peak_us - float(pre_context_us)
    )
    candidate.provisional_start_us = recovered_start
    candidate.final_start_us = recovered_start
    candidate.transient_start_recovered = True
    # This is the first actual candidate, so carry-in collision handling is
    # not applicable even if its first Board pair happened to cross a label.
    candidate.carry_in_start = False
    return True


def _resolve_window_collisions(
    candidates: dict[int, _CandidateWindow],
) -> dict[int, str]:
    """Resolve adjacent contexts and return carry-in candidates that must skip."""

    failures: dict[int, str] = {}
    ordered = [candidates[index] for index in sorted(candidates)]
    for current, following in zip(ordered, ordered[1:], strict=False):
        if following.carry_in_start:
            reason = _resolve_carry_in_start_overlap(current, following)
            if reason is not None:
                failures[following.source_label_index] = reason
            continue
...
def _resolve_remaining_overlaps(candidates: dict[int, _CandidateWindow]) -> None:
    """Final overlap guard after carry-in failures have been removed."""
    ...
    for candidate in (candidates[index] for index in sorted(candidates)):
        if previous is not None and previous.final_end_us > candidate.final_start_us:
            if candidate.carry_in_start:
                reason = _resolve_carry_in_start_overlap(previous, candidate)
                if reason is not None:
                    raise BoardEventSegmentationError(
                        "carry-in segment core overlaps the preceding resolved segment"
                    )
            else:
...
def _resolve_carry_in_start_overlap(
    previous: _CandidateWindow,
    following: _CandidateWindow,
) -> str | None:
    """Keep previous unchanged and resolve only a carry-in segment's start.

    Rules:

    * if ``press - pre_context`` does not enter the previous final segment,
      keep the nominal start;
    * if it enters the previous segment but ``previous.final_end < press``,
      start at the midpoint of ``previous.final_end`` and ``press``;
    * if the previous segment reaches or passes the press itself, the carry-in
      segment cannot retain its event core and is rejected.
    """

    if previous.final_end_us <= following.final_start_us:
        return None
    if previous.final_end_us >= following.first_press_us:
        return "previous_segment_overlaps_carry_in_press"
    following.final_start_us = (
        previous.final_end_us + following.first_press_us
    ) / 2.0
    following.collision_resolved = True
    return None


def _resolve_pair_overlap(
    current: _CandidateWindow, following: _CandidateWindow
) -> None:
    """Keep both event cores and split a normal overlapping context at midpoint."""
...
def _event_targets_for_segment(
    ...
    *,
    ...
    for event in events.itertuples(index=False):
        ...
        if (
            bool(event.incomplete_touch)
            or _optional_int(event.assigned_label_index) != source_label_index
        ):
            continue
        transient = bool(event.transient)
        ...
        local_index = int(
            np.searchsorted(segment_timestamps_us, timestamp, side="left")
        )
...
def _event_neighbors_around_label(
    ordered_events: pd.DataFrame,
    label_timestamp_us: float,
) -> tuple[object | None, object | None]:
    """Return the nearest strict-before and first strict-after Board events."""

    timestamps = ordered_events["aligned_event_timestamp_us"].to_numpy(
        dtype=np.float64
    )
    before = np.flatnonzero(timestamps < label_timestamp_us)
    after = np.flatnonzero(timestamps > label_timestamp_us)
    previous = (
        None
        if len(before) == 0
        else ordered_events.iloc[int(before[-1])]
    )
    following = (
        None
        if len(after) == 0
        else ordered_events.iloc[int(after[0])]
    )
    return previous, following


def _pair_is_immediate_carry_in_around_label(
    ordered_events: pd.DataFrame,
    *,
    paired_touch_index: int,
    label_timestamp_us: float,
) -> bool:
    """Require the same pair's press immediately before and lift immediately after."""

    previous, following = _event_neighbors_around_label(
        ordered_events, label_timestamp_us
    )
    if previous is None or following is None:
        return False
    return (
        str(previous["event_type"]) == "press"
        and _same_touch_index(previous["paired_touch_index"], paired_touch_index)
        and str(following["event_type"]) == "lift"
        and _same_touch_index(following["paired_touch_index"], paired_touch_index)
    )
...
def _classify_crossing_touches(
    ...
    *,
    ...
    The carry-in rule has priority over the historical press-label crossing
    rule.  A complete valid non-transient pair is reassigned to the following
    label only when all of the following hold:

    * ``press < next_label < lift``;
    * ``next_label - press <= carry_in_press_lookback_us``;
    * the nearest Board event before the label is this pair's press; and
    * the first Board event after the label is this pair's lift.

    If the nearest event before the label is a lift, or any other event lies
    between the pair and the label, carry-in is not applied and the historical
    crossing policy decides the pair instead.
    """

    event_table = events.copy(deep=True)
    pair_table = touch_pairs.copy(deep=True)
    boundaries = np.asarray(
        [label.timestamp_us for label in labels], dtype=np.float64
    )
    ...
    ordered_events = event_table.sort_values(
        ["aligned_event_timestamp_us", "event_index"],
        kind="mergesort",
    ...
    for row_index, press_label_index in enumerate(press_label_indices):
        ...
        if (
            next_label_index >= len(labels)
            or lift[row_index] < boundaries[next_label_index]
        ):
            assigned.iloc[row_index] = int(press_label_index)
            continue

        crosses[row_index] = True
        next_label_us = float(boundaries[next_label_index])
        pair_index = _optional_int(pair_table.iloc[row_index]["paired_touch_index"])
        carry_in = (
            pair_index is not None
            and valid_non_transient[row_index]
            and press[row_index] < next_label_us < lift[row_index]
            and 0.0 < next_label_us - press[row_index] <= carry_in_press_lookback_us
            and _pair_is_immediate_carry_in_around_label(
                ordered_events,
                paired_touch_index=pair_index,
                label_timestamp_us=next_label_us,
            )
        )
        if carry_in:
            resolutions[row_index] = _CARRY_IN_RESOLUTION
            assigned.iloc[row_index] = int(next_label_index)
            continue
    ...
    event_table["crosses_next_label_timestamp"] = [
        False
        if pd.isna(pair_index)
        else bool(
            pair_details.loc[pair_index, "crosses_next_label_timestamp"]
        )
        for pair_index in event_table["paired_touch_index"]
    ]
    event_table["crossing_touch"] = event_table[
        "crosses_next_label_timestamp"
    ].astype(bool)
    event_table["crossing_resolution"] = [
        "not_crossing"
        if pd.isna(pair_index)
        else str(pair_details.loc[pair_index, "crossing_resolution"])
        for pair_index in event_table["paired_touch_index"]
    ]
    event_table["assigned_label_index"] = pd.Series(
        [
            pd.NA
            if pd.isna(pair_index)
            else pair_details.loc[pair_index, "assigned_label_index"]
            for pair_index in event_table["paired_touch_index"]
        ],
...
def _validated_standard_event_tables(
    ...
    lift_values = pd.to_numeric(pair_table["lift_timestamp_raw"], errors="coerce")
    complete = ~pair_table["incomplete_touch"].astype(bool)
    if not np.isfinite(lift_values[complete]).all():
        raise BoardEventSegmentationError(
            "complete touch pairs require finite lift timestamps"
        )
    return event_table, pair_table
...
def _validated_aligned_event_tables(
    ...
    _validate_event_types(event_table)
    _finite_series(
        event_table["aligned_event_timestamp_us"], name="aligned event timestamp"
    )
    _finite_series(
        pair_table["aligned_press_us"], name="aligned press timestamp"
    )
    complete = ~pair_table["incomplete_touch"].astype(bool)
    if not np.isfinite(
        pair_table.loc[complete, "aligned_lift_us"].to_numpy(dtype=float)
    ).all():
        raise BoardEventSegmentationError(
            "complete touch pairs require finite aligned lift timestamps"
        )
    if (
...
def _with_crossing_classification_columns(
    ...
    """Accept pre-policy event tables while normalizing audit columns."""
...
def _validated_config(config: BoardEventSegmentationConfig) -> np.dtype:
    if not isinstance(config, BoardEventSegmentationConfig):
        raise BoardEventSegmentationError(
            "config must be BoardEventSegmentationConfig"
        )
    ...
    for value, name in (
        ...
        (config.carry_in_press_lookback_us, "carry_in_press_lookback_us"),
    ):
    ...
    if minimum <= 0.0 or maximum <= 0.0 or minimum > maximum:
        raise BoardEventSegmentationError(
            "label minimum and final-segment maximum must be positive and ordered"
        )
    if config.missing_event_policy != "skip":
        raise BoardEventSegmentationError(
            "only missing_event_policy='skip' is implemented"
        )
    ...
    if config.label_time_domain not in {"ring", "board"}:
        raise BoardEventSegmentationError(
            "label_time_domain must be 'ring' or 'board'"
        )
    if not isinstance(config.require_successful_alignment, bool):
        raise BoardEventSegmentationError(
            "require_successful_alignment must be a boolean"
        )
    if (
        not isinstance(config.minimum_duration_frames, int)
        or config.minimum_duration_frames < 1
    ):
        raise BoardEventSegmentationError(
            "minimum_duration_frames must be a positive integer"
        )
...
def _validated_ring_inputs(
    ...
    try:
        ...
        raise BoardEventSegmentationError(
            "Ring IMU and timestamps must be numeric"
        ) from error
    ...
    if timestamps.ndim != 1 or len(timestamps) != len(imu):
        raise BoardEventSegmentationError(
            "Ring timestamps must be a length-M vector"
        )
    if not np.isfinite(imu).all() or not np.isfinite(timestamps).all():
        raise BoardEventSegmentationError(
            "Ring IMU and timestamps must be finite"
        )
    if np.any(np.diff(timestamps) < 0.0):
        raise BoardEventSegmentationError(
            "Ring timestamps must be nondecreasing"
        )
...
def _resolve_feature_arguments(
    *,
    ...
    """Accept generic feature names while retaining raw-ring API aliases."""
...
def _validated_labels(
    ...
    if not markers:
        raise BoardEventSegmentationError(
            "at least one timestamp label is required"
        )
    previous: float | None = None
    for marker in markers:
        if not isinstance(marker, SegmentLabel):
            raise BoardEventSegmentationError(
                "labels must contain SegmentLabel values"
            )
        ...
        if previous is not None and timestamp <= previous:
            raise BoardEventSegmentationError(
                "label timestamps must be strictly increasing"
            )
        previous = timestamp
    return markers
...
def _validate_label_range(
    labels: Sequence[SegmentLabel], timestamps: np.ndarray
) -> None:
    for marker in labels:
        if marker.timestamp_us < timestamps[0] or marker.timestamp_us > timestamps[-1]:
            raise BoardEventSegmentationError(
                "label timestamp is outside the Ring range"
            )
...
def _canonical_point_for_boundary(
    ...
    *,
    ...
    index = int(
        np.searchsorted(boundary_timestamps_us, timestamp_us, side="left")
    )
...
def _validate_final_samples(
    ...
    for sample in samples:
        if not 0 <= sample.start_sample_index < sample.stop_sample_index_exclusive:
            raise BoardEventSegmentationError(
                "final segmentation window is empty"
            )
...
def _validate_event_types(events: pd.DataFrame) -> None:
    if not events["event_type"].isin(("press", "lift")).all():
        raise BoardEventSegmentationError(
            "Board event_type values must be press or lift"
        )
