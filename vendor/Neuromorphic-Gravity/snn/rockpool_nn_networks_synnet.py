    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, (n_hidden, n_tau) in enumerate(
            ...
            taus = [
                torch.tensor(
                    [bitshift_to_tau(dt, tau_to_bitshift(dt, tau_syn_base)[0] + j)[0].item() for j in range(n_tau)]
                )
                for _ in range(tau_repetitions[i])
            ]
            ...
            # lyr_weights = LinearTorch(shape=(n_channels_in, n_hidden), has_bias=True)
            ################################################################################################
            ...
            self.seq.append(
                neuron_model(
                    ...
                    bias=Constant(0.0),
                    ################################################################################################
                    # threshold=torch.tensor(threshold) if train_threshold else Constant(threshold),
                    threshold=torch.tensor([threshold] * n_hidden) if train_threshold else Constant(threshold),
                    ################################################################################################
                    dt=dt,
                    **default_neuron_kwargs,
                ),
        ...
        # lyr_weights = LinearTorch(shape=(n_hidden, n_classes), has_bias=True)
        ################################################################################################
        with torch.no_grad():
