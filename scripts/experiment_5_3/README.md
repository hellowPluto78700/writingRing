...
| `lif_tau242` | one long-membrane LIF layer | beta = 0.9375 (shift_mem = 4) |
...
The two pure-LIF controls differ only in membrane decay; `lif_tau242` uses repository-style `shift_mem=4`:

```text
lif_tau242: beta = 0.9375, shift_mem = 4, tau_mem about 242 ms at 64 Hz
