...
# SNN-only probe BA across all three training seeds. Test is never accessed here.
# -----------------------------------------------------------------------------
...
for condition in CONTRASTIVE_CONDITIONS:
    for lambda_con in LAMBDA_CON_GRID:
        ...
        for seed in SEEDS:
            ...
            probe = fit_logreg_probe(ztr250, zva250, None, seed_tag=("dev", condition, lambda_con, seed))
            seed_probe_bas.append(probe["val"]["balanced_accuracy"])
...
# -----------------------------------------------------------------------------
# Final confirmation. Only after lambda selection do we evaluate test.
# -----------------------------------------------------------------------------
FINAL_CONDITIONS = {
    "cls_only": 0.0,
...
# Raw baseline with the same validation-selected Logistic Regression protocol.
raw_probe = fit_logreg_probe(RAW250["train"], RAW250["val"], RAW250["test"], seed_tag="raw250")
RAW_TEST_BA = float(raw_probe["test"]["balanced_accuracy"])

for condition, lambda_con in FINAL_CONDITIONS.items():
    for seed in SEEDS:
        ckpt = CHECKPOINT_DIR / f"final_{condition}_seed_{seed}.pt"
        payload = train_run(condition, seed, lambda_con, ckpt)
        model = model_from_payload(payload)
        ...
        with torch.no_grad():
            test_metrics = run_epoch(model.to(DEVICE), make_loader("test", seed, False), condition, lambda_con, None)
        ...
        for readout, arrays in (("SNN250", (ztr250, zva250, zte250)), ("SNN125", (ztr125, zva125, zte125))):
            probe = fit_logreg_probe(*arrays, seed_tag=("final", readout, condition, seed))
            probe_rows.append({
