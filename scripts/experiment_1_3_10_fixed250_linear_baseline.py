...
# Importing the runner first applies the current Experiment 1.3.10 protocol
# patch (including PROTOCOL_VERSION) to the shared core module. This keeps the
# paired Fixed250+Linear baseline in the same artifact tree as the SNN runs.
import experiment_1_3_10_runner as protocol
...
def build_fixed250_count_features(
    cohort,
    frame: pd.DataFrame,
    ...
    if padded_steps > GLOBAL_PADDED_LENGTH:
        events = np.pad(
            events,
            ((0, 0), (0, padded_steps - GLOBAL_PADDED_LENGTH), (0, 0)),
        )
    ...
    meta: dict[str, int | float | str] = {
        ...
        "aggregation": (
            "per-channel sum within each fixed 250 ms bin, then flatten bins "
            "in absolute order"
        ),
    }
...
def baseline_path(
    ...
    return (
        results_dir(repo_root, labels)
        / "baselines"
        / f"{BASELINE_ID}_split{split_seed}.json"
    )
...
def run_one_split(
    ...
    x_train, y_train, representation = build_fixed250_count_features(
        cohort, parts["train"]
    )
...
def write_summary(
    ...
    frame = (
        pd.DataFrame(flatten(payload) for payload in payloads)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Paired Fixed250 count + Linear baseline for Experiment 1.3.10"
    )
    parser.add_argument(
        "--labels",
        type=str,
...
def main() -> None:
    ...
    split_seeds = (
        (args.split_seed,) if args.split_seed is not None else SPLIT_SEEDS
    )
    payloads = [
        run_one_split(repo_root, labels, split_seed, overwrite=args.overwrite)
